...
# ============================================================
#  DATABASE METHODS  (apni DB class mein paste karo)
# ============================================================
...
class VerificationDB:
    ...
    async def setup_indexes(self):
        """
        MongoDB TTL indexes lagao taaki tokens automatically delete hon.
        Bot ke startup mein ek baar call karo:
            await db.setup_indexes()
        """
        # Unverified tokens — 24hr baad auto-delete
        await self.verify_id.create_index(
            "created_at",
            expireAfterSeconds=86400,       # 24 hours
            name="token_ttl_index",
        )
        # Verified tokens — 48hr baad auto-delete
        # sparse=True → sirf wahi documents jahan verified_at field ho
        await self.verify_id.create_index(
            "verified_at",
            expireAfterSeconds=172800,      # 48 hours
            name="verified_token_ttl_index",
            sparse=True,
        )
    ...
    async def get_notcopy_user(self, user_id):
        ...
        if not user:
            res = {
                ...
                "bypass_attempts": 0,       # Total bypass attempts (lifetime)
                "bypass_history": [],       # Last 7 din ki history
            }
    ...
    # ----------------------------------------------------------
    #  Token (verify_id) — create / fetch / update / invalidate
    # ----------------------------------------------------------
    ...
    async def get_existing_verify_id(self, user_id: int):
        ...
        Sirf wahi token return karo jo 24 ghante se purana na ho.
        """
        ist_timezone = pytz.timezone(TIMEZONE)
    ...
    async def invalidate_token(self, user_id: int, hash: str):
        """
        Bypass detect hone par token turant invalidate karo.
        verified_at bhi save karo taaki 48hr TTL kaam kare aur
        DB mein indefinitely na pada rahe.
        """
        ist_timezone = pytz.timezone(TIMEZONE)
        current_time = datetime.datetime.now(tz=ist_timezone)
        await self.update_verify_id_info(user_id, hash, {
            "verified": True,
            "verified_at": current_time,
            "invalidated_by": "bypass_detection",   # audit trail
        })
    ...
    async def log_bypass_attempt(self, user_id: int, seconds_taken: int) -> int:
        """
        User ka bypass attempt DB mein save karo.
        Return karo total bypass attempts count.
        """
        ist_timezone = pytz.timezone(TIMEZONE)
        current_time = datetime.datetime.now(tz=ist_timezone)

        # Pehle 7 din se purani history clean karo
        await self.clean_old_bypass_history(user_id)

        # Naya attempt record karo
        ...
        user = await self.get_notcopy_user(user_id)
        return user.get("bypass_attempts", 0)

    async def clean_old_bypass_history(self, user_id: int):
        """7 din se purani bypass history entries remove karo."""
        ist_timezone = pytz.timezone(TIMEZONE)
        week_ago = datetime.datetime.now(tz=ist_timezone) - datetime.timedelta(days=7)
        await self.misc.update_one(
            {"user_id": user_id},
            {"$pull": {"bypass_history": {"at": {"$lt": week_ago}}}},
        )

    async def reset_bypass_attempts(self, user_id: int):
        """Admin use: kisi user ke bypass attempts reset karo."""
        await self.misc.update_one(
            {"user_id": user_id},
            {"$set": {"bypass_attempts": 0, "bypass_history": []}},
        )
...
async def _get_or_create_token(user_id: int) -> str:
    """
    Agar user ka koi unverified + unexpired token pehle se hai to wahi return karo,
...
async def start(client: Client, message):
    ...
    if len(m.command) == 2 and m.command[1].startswith(("kdbotz", "notcopy")):
        ...
        # Threshold DB se lo — admin /setbypass command se change kar sakta hai
        BYPASS_THRESHOLD = int(await db.get_setting("bypass_threshold", 60))
        ...
        if created_at:
            ...
            if time_since_creation < BYPASS_THRESHOLD:

                # Step 1: Token turant invalidate karo — dobara use na ho
                await db.invalidate_token(verify_user_id, verify_id)

                # Step 2: Bypass attempt log karo, total count lo
                attempts = await db.log_bypass_attempt(verify_user_id, int(time_since_creation))

                # Step 3: Repeat offender warning
                repeat_warning = ""
                if attempts >= 3:
                    repeat_warning = (
                        f"\n🚨 <b>Repeated Bypass!</b> Ye user <b>{attempts} baar</b> "
                        f"bypass try kar chuka hai!"
                    )

                # Step 4: Log channel mein detail bhejo
                await client.send_message(
                    ...
                    f"(threshold: {BYPASS_THRESHOLD}s)\n"
                    f"🔁 Total bypass attempts: <b>{attempts}</b>\n"
                ...
                return  # Token invalidate ho gaya — user naya token lega
        ...
        # verified_at save karo taaki 48hr TTL kaam kare
        await db.update_verify_id_info(verify_user_id, verify_id, {
            "verified": True,
            "verified_at": current_time,
        })
